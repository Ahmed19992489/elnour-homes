...
5: type NotificationEvent = "status_changed" | "customer_cancelled";

type OrderNotificationOptions = { includeInApp?: boolean };
8: const statusCopy: Record<string, { ar: string; en: string }> = {
...
17: function escapeHtml(value: string) {

return value.replace(/[&<>'"]/g, char => ({
"&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;",
}[char] || char));
}
23: export function getNotificationCopy(order: Order, event: NotificationEvent) {
...
? `مرحبًا ${name}، تم استلام طلب إلغاء
: `مرحبًا ${name}، ${state.ar}. رقم الطلب #${order.id}${order.productName ? ` — ${order.productName}` : ""}.`;
return { title, message, statusLabel: state.ar };
}
35: /** Creates the customer-visible site notification before any external delivery attempt. */
...
const copy = getNotificationCopy(order, event);
60:   if (options.includeInApp !== false) {

await createOrderInAppNotification(order, event);
}
64:   const account = order.userId ? await db.getUserById(order.userId) : undefined;

const recipient = order.customerEmail || account?.email || undefined;
const sender = process.env.RESEND_FROM_EMAIL;
68:   if (!recipient || !sender || !process.env.RESEND_API_KEY) {
...
83:   const origin = process.env.CANONICAL_ORIGIN || "https://elnoursteel-eexiztdb.manus.space";

...
function parseProductOptions(value?: string | null) {
...
/** Reset the master-admin cache — used by tests to isolate each scenario. */
export function __resetMasterAdminCache(): void {
  masterAdminOpenIdCache = { value: null, at: 0 };
}

async function getMasterAdminOpenId(): Promise<string | null> {
  const now = Date.now();
...
async function isOwnerOrMasterAdmin(user: { openId: string | null } | null, ownerOpenId: string): Promise<boolean> {
...
function isAvailableOption(options: string[], selected?: string) {
  ...
  orders: router({
    create: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Track referral code if provided (must belong to another existing customer)
    ...
    createCart: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // 3. Build a human-readable summary for the single order record
        ...
        // 6. In-app + customer notifications
        if (orderResult[0]) {
          const orderId = typeof orderResult[0].insertId === "number" ? orderResult[0].insertId : undefined;
          if (orderId) {
            await db.updateOrderNotificationStatus(orderId, notificationSent);
        ...
        return { success: true, notificationSent, orderId: typeof orderResult[0]?.insertId === "number" ? (orderResult[0].insertId as number) : undefined, subtotal, discount: couponApplied.discount };
      }),
  ...
  adminAuth: router({
    ...
    login: publicProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const credential = await db.getAdminCredentialByPhone(normalized);
        ...
        // Invalidate stale sessions for this phone so only one device stays signed in.
  ...
  adminAccounts: router({
    ...
    updatePassword: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        const hash = await bcrypt.hash(input.password, 10);
        await db.upsertAdminCredential({ phone: normalized, passwordHash: hash, displayName: existing.displayName });
        return { success: true };
      }),
